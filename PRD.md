...
- ⚠️ CI's compilation coverage differs by platform. Android: `assembleDebug` compiles the actual `LiveActivityModule.kt` against the example app. iOS: `swift test --package-path tests/ios` compiles and tests only the standalone `LiveActivityContentParser.swift` (deliberately free of React/ActivityKit imports, reached via a symlink), and `pod install` generates a workspace but compiles nothing — so **nothing in CI compiles the post-extraction `ios/LiveActivityModule.swift`**, the file whose parsing body was gutted and rewired on this branch. Swift and Kotlin parser tests plus JS tests (mocked native module) do run and pass on every push. The iOS module's compilation is first checked by the device-verification step, not by CI. This is not a claim that the code is broken — it reads correct — only that it is unverified by a compiler.
- ✅ Example app running the start/update/end lifecycle on real hardware — confirmed by the repository owner at commit `a4edbd1`. This also closes the compiler gap noted above: the device build is the first thing to compile the post-extraction `ios/LiveActivityModule.swift`. Recorded in [`docs/plans/2026-08-01-device-verification.md`](./docs/plans/2026-08-01-device-verification.md).
- ✅ Native countdown timer is implemented for iOS — parsed, validated, and wired to render live via the widget's `Text(timerInterval:)`. The iOS code now compiles and runs on hardware, but the countdown's own state transitions (pause → resume → complete) were not among the behaviours recorded during device verification. Android's native module does not read `content.timer` at all, so it has zero rendering effect there (see README **Platform behavior differences**).
...
- ⚠️ Real-device verification is **partial**. The core lifecycle was exercised on hardware and found working, but per-item results were not recorded, so the following remain unverified: Dynamic Island's three presentations, the countdown's pause → resume → complete transitions, `getActiveActivities()` reconnection after a process restart, the per-platform unknown-`activityId` divergence, the foreground service's one-at-a-time limit and Android 14 `dataSync` startup, `isSupported()`'s second gate, and the README-only Widget Extension setup path. The README's six platform-difference rows were all derived by reading code and still have no physical confirmation. See the verification record for the full list.
