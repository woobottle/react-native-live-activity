...
**검증자:** woobottle (저장소 소유자)
**날짜:** 2026-08-01
**라이브러리 커밋:** `a4edbd1`

> ### ⚠️ 이 검증은 부분 검증이다 — 기록의 정확한 범위
>
> 소유자가 실기기에서 example 앱을 직접 실행해 **핵심 lifecycle(start / update /
> end)이 동작함을 확인**했다. 이로써 이 릴리즈의 가장 큰 미지수였던
> **`ios/LiveActivityModule.swift`의 컴파일**이 해소됐다 — 이 브랜치에서 파싱
> 본문을 통째로 들어낸 파일인데 CI의 iOS 잡은 독립 파서만 컴파일하고
> `pod install`은 아무것도 컴파일하지 않으므로, 실기기 빌드가 이 파일을 컴파일한
> 최초의 사례다.
>
...
- [x] **v0.1.0 릴리즈 가능 — 단, 부분 검증 기준.** 실기기에서 핵심 lifecycle이
      동작했고 블로커는 발견되지 않았다. 문서 상단의 "부분 검증" 박스에 열거한
      미검증 항목이 남아 있음을 알고 내보내는 결정이다.
- [ ] 블로커 있음 — 위 표 참조

### 남은 일

- 위 미검증 항목을 0.1.x 동안 채운다. 특히 README 플랫폼 차이표 6개 행은
  전부 코드를 읽어 도출한 것이라 실물 대조가 아직 없다.
- `(배포 후)` 표시가 붙은 6개 항목은 npm 배포 이후에만 수행할 수 있다.
